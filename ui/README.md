...
1. 仪表盘 Dashboard — 点数余额、本月用量、共享收益、点数来源（点数构成分组）
...
4. 钱包 Wallet — 点数余额、点数来源（点数构成分组）、**充值入口（US-4：模拟流程——输入点数 → 余额增加 → topup 交易记录；文案注明"演示，真实支付后续接入"；提现仍 disabled）**、**申请加额（US-20：企业成员余额低时申请更多点数 → 提交后等待管理员审批，与组织设置开关联动）**；收支明细已去重，统一到【交易记录】（页内提供跳转提示）
