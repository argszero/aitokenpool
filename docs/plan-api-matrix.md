...
内置 DeepSeek / GLM / GPT / Claude / Gemini / 豆包 / MiniMax / 通义等模型，含 context window、vision 支持、缓存价与高峰价字段。**数量与明细以 `config/config.example.toml` 的 `[[models]]` 为唯一真源**（由 `src/catalog_gate.rs` 的 `MODEL_COUNT` 守卫，此处不抄写数字以免随之陈旧）。模型列表可经管理端「模型管理」页 CRUD（需 admin 角色）。
