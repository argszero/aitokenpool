...
后端 + 前端已全部实现，当前版本 **v0.4.1**（94/94 测试全绿）：
...
- `GET /healthz` → `{"status":"ok","version":"0.4.1"}`
...
- `POST /v1/chat/completions` / `POST /anthropic/v1/messages` / `POST /v1/responses`（网关，三协议互转；SSE 流式同协议透传）；`GET /v1/models`（OpenAI 兼容模型列表，认证可选）
- `GET /api/models`（模型市场）
